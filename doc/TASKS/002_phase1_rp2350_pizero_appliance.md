...
2) Ensure you are on the default branch and it is clean:
   ...
   - If there are uncommitted changes, DO NOT DISCARD THEM:
     - create a WIP stash: git stash push -u -m "WIP before phase1 rp2350 pizero"
...
src/
  board_pins.h
  main.c
  FreeRTOSConfig.h                  (REQUIRED; FreeRTOS expects this exact name)
  log.c
  log.h
  status.c
  status.h
  ...
  video/
    ...
    tmds_pio.c
    tmds_pio.h
    tmds.pio                       (REQUIRED PIO program source for TMDS)
    patterns.c
    patterns.h
    osd.c
    osd.h
  ...
  usb/
    ...
    usb_host_piousb.c
    usb_host_piousb.h
    piousb_host.pio                 (REQUIRED PIO program source for PIO-USB host)
    hid_keyboard.c
    hid_keyboard.h
  ...
  storage/
    ...
    ff_port.c
    ff_port.h
    ffconf.h                        (REQUIRED; FatFS configuration header must exist)
    splash.c
    splash.h
...
third_party/ (ONLY what is required by Sections E/Q/S; include licenses; no placeholders; no empty dirs)
  README_THIRD_PARTY.md             (lists vendored/submoduled components + licenses)
  LICENSES/                         (copies of all third-party licenses referenced above)
...
Examples of required files that must exist if referenced:
- pico_sdk_import.cmake
- FreeRTOSConfig.h
- ffconf.h
- any *.pio sources listed in section L/Q8
- tools/*.py and scripts/* listed in section L
- assets/sdroot/splash.raw, VERSION.TXT, assets/sdroot.zip

R5) Submodules and vendoring handling (no guessing)
If the build fails due to missing third_party code:
- First check if the repo already uses git submodules:
  - git submodule status
  - If submodules exist, initialize/update them:
    git submodule update --init --recursive
- If no submodules exist OR they do not provide the needed dependency, vendor the minimal required code under third_party/ and include licenses (as already required by section L and Section S).
- Record which path was used in README.md under “Dependencies”.
...
- FreeRTOS kernel sourcing MUST be deterministic:
  - Prefer FreeRTOS-Kernel as committed sources under: third_party/FreeRTOS-Kernel/
