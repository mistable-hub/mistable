...
## 0) IMPORTANT EXECUTION NOTE (Codex CLI)
This task is meant to be executed **inside an interactive Codex session** (TTY).  
**Do NOT** attempt `codex chat < file.md` (it fails: “stdin is not a terminal”).

Use ONE of these methods:

**Method A (recommended):**
1) Run `codex chat`
2) Paste this entire task file content into Codex
3) Tell Codex: “Execute exactly; follow all constraints.”

**Method B (single command prompt arg):**
- Run:
  - `codex "$(cat doc/TASKS/002_phase1_rp2350_pizero_appliance.md)"`

If Method B fails due to shell/arg limits, fall back to Method A.

---

## A) GIT / PR WORKFLOW (MANDATORY FIRST STEPS)

### A1) Detect repo default branch
1) Detect the repo default branch:
   - If `main` exists use `main`, else if `master` exists use `master`.

### A2) Ensure default branch is clean
...
## A5) SUBMODULES (MANDATORY, FIXES FUNDAMENTAL FAILURES)
**This project depends on submodules being present at build time.**  
The most common “fundamental” failure mode so far is: **submodules exist in `.gitmodules` but are not initialized**, causing host-compiler builds and missing Pico SDK headers/toolchain wiring.

Immediately after creating the feature branch, ALWAYS run:

- `git submodule sync --recursive`
- `git submodule update --init --recursive`

Then verify the Pico SDK submodule is actually populated:

- `test -f third_party/pico-sdk/pico_sdk_init.cmake`
- `test -f third_party/pico-sdk/external/pico_sdk_import.cmake`

If either file is missing:
- STOP and fix submodules. Do NOT proceed.
...
If Waveshare baseline sources are not present locally, follow Section S4 (deterministic DVI/TMDS selection);  
do NOT “mirror architecture” loosely.
...
`third_party/` (ONLY what is required; include licenses; no placeholders; no empty dirs)
- `README_THIRD_PARTY.md`
- `LICENSES/`
- `waveshare_demo/` (submodule preferred; must be populated)
- `pico-sdk/` (submodule required; must be populated)
- `FreeRTOS-Kernel/` (submodule required; must be populated)
- `PicoDVI_or_TMDS/` (only if Waveshare DVI demo absent)
- `FatFS/` (only if Pico SDK doesn’t provide usable FatFS integration)
...
### Q10) Build must refuse to proceed if submodules are missing (NEW, MANDATORY)
To prevent the repeated “fundamental” failures:
- `scripts/build.sh` MUST verify required submodule files exist BEFORE running cmake:
  - `third_party/pico-sdk/pico_sdk_init.cmake`
  - `third_party/FreeRTOS-Kernel/include/FreeRTOS.h` (or another deterministic kernel header)
  - `third_party/waveshare_demo/README.md` and required subpaths if used
- If missing, print:
  - `git submodule update --init --recursive`
  and exit non-zero.
...
- First check submodules:
  ...
  - `git submodule update --init --recursive`
- If submodules exist but aren’t populated: fix that first.
- Vendor ONLY if explicitly required by S4/S6 fallback paths.
...
- `third_party/waveshare_demo/README.md` MUST exist and MUST include:
  - the exact upstream origin (URL)
  - the exact archive name or tag
  - the date retrieved
  - if possible, a commit hash or version identifier
...
### S2) Pico SDK version pinning (MANDATORY) — SUBMODULE ONLY
- The project MUST use a pinned Pico SDK revision.
- Deterministic rule:
  - Pico SDK MUST be a git submodule at: `third_party/pico-sdk/`
- After initializing submodules, record the exact Pico SDK commit SHA in `README.md`:
  - `git -C third_party/pico-sdk rev-parse HEAD`

### S3) FreeRTOS kernel source pinning (MANDATORY) — SUBMODULE ONLY
- FreeRTOS kernel sourcing MUST be deterministic:
  - FreeRTOS-Kernel MUST be a git submodule at: `third_party/FreeRTOS-Kernel/`
- Record the exact commit SHA in `README.md`:
